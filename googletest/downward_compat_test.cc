...
TEST(DownwardCompatibilityTest, DownwardCompatibility) {
  for (auto const i : {1, 2}) {
    ...
    if (cista::unchecked_deserialize<version_detection>(buf)->version_ == 1) {
      ...
      EXPECT_EQ(v1->values_[0]->s_, "A");
      EXPECT_EQ(v1->values_[1]->s_, "B");
      EXPECT_EQ(v1->values_[2]->s_, "C");
    } else if (cista::unchecked_deserialize<version_detection>(buf)->version_ ==
               2) {
      auto const v2 = cista::unchecked_deserialize<data_v2>(buf);
      EXPECT_EQ(v2->values_[0]->s1_, "A");
      EXPECT_EQ(v2->values_[1]->s1_, "B");
      EXPECT_EQ(v2->values_[2]->s1_, "C");
    }
  }
}
