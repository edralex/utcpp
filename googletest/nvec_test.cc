...
TEST(NvecTest, Nvec) {
...

  {
    ...
    EXPECT_TRUE(std::equal(begin(bucket), end(bucket), begin(entries), end(entries)));
  }
  ...
  EXPECT_EQ(2U, v.size(0U));
  EXPECT_EQ(3U, v.size(1U));
  EXPECT_EQ(2U, v.size(0U, 0U));
  EXPECT_EQ(3U, v.size(0U, 1U));
  EXPECT_EQ(2U, v.size(1U, 0U));
  EXPECT_EQ(3U, v.size(1U, 1U));
  EXPECT_EQ(1U, v.size(1U, 2U));
  EXPECT_EQ(2U, v.size(2U, 0U));
  EXPECT_EQ(3U, v.size(2U, 1U));
  EXPECT_EQ(1U, v.size(2U, 2U));

  EXPECT_EQ(2U, v[0U].size());
  EXPECT_EQ(3U, v[1U].size());
  EXPECT_EQ(2U, v[0U][0U].size());
  EXPECT_EQ(3U, v[0U][1U].size());
  EXPECT_EQ(2U, v[1U][0U].size());
  EXPECT_EQ(3U, v[1U][1U].size());
  EXPECT_EQ(1U, v[1U][2U].size());
  EXPECT_EQ(2U, v[2U][0U].size());
  EXPECT_EQ(3U, v[2U][1U].size());
  EXPECT_EQ(1U, v[2U][2U].size());
}
