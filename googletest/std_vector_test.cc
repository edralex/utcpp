...
TEST(VectorSerializationTest, VectorTest) {
  ...
  EXPECT_EQ(serialized[0][0][0], 1);
  EXPECT_EQ(serialized[0][1][0], 2);
  EXPECT_EQ(serialized[0][2][0], 3);
  EXPECT_EQ(serialized[1][0][0], 4);
  EXPECT_EQ(serialized[1][1][0], 5);
  EXPECT_EQ(serialized[2][0][0], 6);

  // Manual destruction due to placement-new in deserialization
...
TEST(VectorSerializationTest, VectorTupleTest) {
  ...
  EXPECT_EQ(0, std::strncmp("what?", cista::get<3>(*tuple).data(), 5));
