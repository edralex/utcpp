...
TEST(Bitvec, Bignum) {
  ...
  uut.for_each_set_bit([](auto const i) { EXPECT_EQ(i, 8355212021ULL); });
}

TEST(Bitvec, LessThan) {
  auto const str_1 =
  ...
  EXPECT_EQ(count, uut2.count());

  EXPECT_EQ(uut1.str(), std::string_view{str_1});
  EXPECT_EQ(uut2.str(), std::string_view{str_2});
  EXPECT_EQ(to_string(ref1), std::string_view{str_1});
  EXPECT_EQ(to_string(ref2), std::string_view{str_2});

  auto const ref_lt = bitvec_lt(ref1, ref2);
  auto const uut_lt = bitvec_lt(uut1, uut2);
  EXPECT_EQ(ref_lt, uut_lt);
}

TEST(Bitvec, AtomicSet) {
...
TEST(Bitvec, Parallel) {
  ...
  EXPECT_EQ(bits, check);
}

TEST(Bitvec, ParallelMarkStore) {
