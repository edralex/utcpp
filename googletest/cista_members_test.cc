...
TEST(ToTupleTest, CistaMembersToTuple) {
  struct x {};
  struct a : public x {
  ...
  auto const obj = a{};
  EXPECT_EQ(std::get<0>(cista::to_tuple(obj)), 1);
  EXPECT_EQ(std::get<1>(cista::to_tuple(obj)), 2);
  EXPECT_EQ(std::get<2>(cista::to_tuple(obj)), 3);
  static_assert(
...
TEST(PrintableTest, CistaMembersPrintable) {
  std::stringstream ss;
  ss << a{};
  EXPECT_EQ(ss.str(), "{1, 2, 3}");
}

TEST(PrintableTest, CistaMembersPrintableNames) {
  std::stringstream ss;
  ss << b{};
...
TEST(SerializationTest, CistaMembersSerialization) {
  cista::byte_buf buf;
