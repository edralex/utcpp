...
#include <thread>
#include <vector>
#include <algorithm> // Добавлено для std::generate, std::sort, std::unique
...
TEST_CASE("bitvec bignum") {
  ...
  // CHECK_EQ заменен на CHECK( == )
  uut.for_each_set_bit([](auto const i) { CHECK(i == 8355212021ULL); });
}
...
TEST_CASE("bitvec parallel") {
  ...
  // CHECK_EQ заменен на CHECK( == )
  // Для корректного вывода векторов при ошибке Catch2 требуется оператор <<
  // или специализация StringMaker, но стандартные векторы часто поддерживаются по умолчанию.
  CHECK(bits == check);
}
