...
TEST_CASE("vector test") {
  ...
  REQUIRE(serialized[0][0][0] == 1);
  REQUIRE(serialized[0][1][0] == 2);
  REQUIRE(serialized[0][2][0] == 3);
  REQUIRE(serialized[1][0][0] == 4);
  REQUIRE(serialized[1][1][0] == 5);
  REQUIRE(serialized[2][0][0] == 6);
...
TEST_CASE("vector tuple test") {
  ...
  REQUIRE(0 == std::strncmp("what?", cista::get<3>(*tuple).data(), 5));
