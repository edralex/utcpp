...
TEST_CASE("cista_members to tuple", "[reflection]") {
  struct x {};
  struct a : public x {
...
TEST_CASE("cista_members printable", "[reflection][printable]") {
  std::stringstream ss;
  ss << a{};
  CHECK(ss.str() == "{1, 2, 3}");
}

TEST_CASE("cista_members printable names", "[reflection][printable]") {
  std::stringstream ss;
  ss << b{};
...
// Опционально: определение статического члена (если используется в сериализации, но не обязательно для этого теста)
// int serialize_me::s_ = 0;

TEST_CASE("cista_members serialization", "[reflection][serialization]") {
  cista::byte_buf buf;
