...
#define RTreeTest RTreeTestSuite

TEST(RTreeTest, SortByAxis) {
  int N = 10;
...
TEST(RTreeTest, NewNode) {
  ...
  EXPECT_EQ(rt.nodes_.size(), 1U);
  rt.node_new(cista::rtree<size_t>::kind::kLeaf);
  EXPECT_EQ(rt.nodes_.size(), 2U);
}

TEST(RTreeTest, MoveFromNodeToNode) {
  int N = 10;
  std::vector<cista::rtree<size_t>::rect> rand_rects_list;
  for (int i = 0; i < N; ++i) {
    fill_rand_rect(rand_rects_list);
  ...
  EXPECT_EQ(rt.get_node(other_node).count_, 3);
  EXPECT_EQ(rt.get_node(rt.m_.root_).count_, 7);
}

TEST(RTreeTest, RandomInsert) {
  int N = 100000;
  std::vector<cista::rtree<size_t>::rect> rand_rects_list;
...
TEST(RTreeTest, SimpleDeleteDepth1) {
  int N = 10;
  std::vector<cista::rtree<size_t>::rect> rand_rects_list;
  srand(static_cast<unsigned>(time(nullptr)));
  ...
  EXPECT_FALSE(found_correct);
}

TEST(RTreeTest, SimpleDeleteDepth2) {
  int N = 70;
  std::vector<cista::rtree<size_t>::rect> rand_rects_list;
  srand(static_cast<unsigned>(time(nullptr)));
  ...
  temp_id.v_ = 2;
  // Note: This access assumes specific tree structure which might be flaky with random data
  // but kept as is from original test logic.
...
TEST(RTreeTest, InsertAfterDelete) {
  int N = 10000;
  std::vector<cista::rtree<size_t>::rect> rand_rects_list;
  srand(static_cast<unsigned>(time(nullptr)));
  ...
  EXPECT_EQ(tree_size_after_deletion, rt.nodes_.size());
}

TEST(RTreeTest, CustomDataStruct) {
  uint32_t N = 1000;
  srand(static_cast<unsigned>(time(nullptr)));
...
TEST(RTreeTest, MultipleDimsRectangle) {
  int N = 1000;
  srand(static_cast<unsigned>(time(nullptr)));
...
TEST(RTreeTest, ReadWriteFile) {
  ...
  EXPECT_TRUE(rt.m_.rect_.equals(rt_uut.m_.rect_));
  EXPECT_EQ(rt.m_.root_.v_, rt_uut.m_.root_.v_);
  EXPECT_EQ(rt.m_.free_list_.v_, rt_uut.m_.free_list_.v_);
  EXPECT_EQ(rt.m_.count_, rt_uut.m_.count_);
  EXPECT_EQ(rt.m_.height_, rt_uut.m_.height_);
  EXPECT_EQ(rt.m_.path_hint_, rt_uut.m_.path_hint_);
}