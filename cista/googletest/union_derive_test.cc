...
TEST(UnionTest, BasicUnion) {
  ...
  EXPECT_FLOAT_EQ(deserialize->b_, 33.4F);
}
...
TEST(UnionTest, DerivedClass) {
  ...
  ASSERT_NE(deserialize, nullptr);
  EXPECT_EQ(deserialize->field, 33);
}
...
TEST(UnionTest, ComplexUnion) {
  ...
  // Hash map iteration order is not guaranteed, so check both possible orders
  bool check = (result == "1, 2\n3, 4\n") || (result == "3, 4\n1, 2\n");
  EXPECT_TRUE(check);
}