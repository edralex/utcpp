...
TEST(OffsetIndexedVecTest, SerializeFile) {
  ...
  unsigned i = 0;
  EXPECT_EQ((*std::next(begin(visited), i++))->name_, data::string{"NODE A"});
  EXPECT_EQ((*std::next(begin(visited), i++))->name_, data::string{"NODE B"});
  EXPECT_EQ((*std::next(begin(visited), i++))->name_, data::string{"NODE C"});
  EXPECT_EQ(*g->node_names_[0], "NODE A");
  EXPECT_EQ(*g->node_names_[1], "NODE B");
  EXPECT_EQ(*g->node_names_[2], "NODE C");
}

TEST(OffsetIndexedVecTest, SerializeBuf) {
  constexpr auto const MODE =
      cista::mode::WITH_INTEGRITY | cista::mode::WITH_VERSION;
...
TEST(OffsetIndexedVecTest, SerializeMmapFile) {
...

  {
    ...
    EXPECT_EQ(mmap.checksum(), CHECKSUM_INTEGRITY_AND_VERSION);
  }  // EOL graph
  ...
  EXPECT_EQ((*std::next(begin(visited), i++))->name_, data::string{"NODE B"});
  EXPECT_EQ((*std::next(begin(visited), i++))->name_, data::string{"NODE C"});
  EXPECT_EQ(*g->node_names_[0], data::string{"NODE A"});
  EXPECT_EQ(*g->node_names_[1], data::string{"NODE B"});
  EXPECT_EQ(*g->node_names_[2], data::string{"NODE C"});
}

TEST(OffsetIndexedVecTest, SerializeEndianTest) {
...

  {
    ...
    mmap.buf_.reserve(512);
    cista::serialize<MODE>(mmap, g);
    EXPECT_EQ(mmap.checksum(), CHECKSUM_BIG_ENDIAN);
  }  // EOL graph
