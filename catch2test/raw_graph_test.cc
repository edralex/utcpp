...
TEST_CASE("graph raw serialize file") {
...

  {
    ...
    REQUIRE(f.checksum() == CHECKSUM_INTEGRITY_AND_VERSION);
  }  // EOL graph

  auto b = cista::file(FILENAME, "r").content();
  REQUIRE(cista::hash(b) == CHECKSUM_INTEGRITY_AND_VERSION);

  auto const g = cista::deserialize<graph, MODE>(b);
  auto const visited = bfs(g->nodes_[0].get());
  unsigned i = 0;
  REQUIRE((*std::next(begin(visited), i++))->name_ == data::string{"NODE A"});
  REQUIRE((*std::next(begin(visited), i++))->name_ == data::string{"NODE B"});
  REQUIRE((*std::next(begin(visited), i++))->name_ == data::string{"NODE C"});
}
...
TEST_CASE("graph raw serialize buf") {
  ...
  cista::byte_buf buf;
  {
    ...
    REQUIRE(b.checksum() == CHECKSUM_INTEGRITY_AND_VERSION);

    buf = std::move(b.buf_);
  }  // EOL graph

  REQUIRE(cista::hash(buf) == CHECKSUM_INTEGRITY_AND_VERSION);
