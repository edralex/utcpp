...
TEST_CASE("JsonComplexObject parse complex object", "[json][complex]") {
    json::jobject test_parse = json::jobject::parse(test_data);
    REQUIRE(test_parse.size() == 22);
}

TEST_CASE("JsonComplexObject serialize and reparse", "[json][complex]") {
    json::jobject test_parse = json::jobject::parse(test_data);
    const std::string echo_str = test_parse.as_string();
    json::jobject echo = json::jobject::parse(echo_str);
    REQUIRE(echo.size() == 22);
}
