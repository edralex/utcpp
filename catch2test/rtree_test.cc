...
TEST_CASE("rtree move from node to node") {
  int N = 10;
  std::vector<cista::rtree<size_t>::rect> rand_rects_list;
  for (int i = 0; i < N; ++i) {
    fill_rand_rect(rand_rects_list);
...
TEST_CASE("rtree simple delete: depth 1") {
  int N = 10;
  std::vector<cista::rtree<size_t>::rect> rand_rects_list;
  srand(static_cast<unsigned>(time(nullptr)));
...
TEST_CASE("rtree simple delete: depth 2") {
  int N = 70;
  std::vector<cista::rtree<size_t>::rect> rand_rects_list;
  srand(static_cast<unsigned>(time(nullptr)));
...
TEST_CASE("rtree insert after delete") {
  int N = 10000;
  std::vector<cista::rtree<size_t>::rect> rand_rects_list;
  srand(static_cast<unsigned>(time(nullptr)));
...
TEST_CASE("rtree custom data struct") {
  uint32_t N = 1000;
  srand(static_cast<unsigned>(time(nullptr)));
...
TEST_CASE("rtree multiple dims rectangle") {
  int N = 1000;
  srand(static_cast<unsigned>(time(nullptr)));
...
TEST_CASE("rtree read/write file") {
  int N = 1000;
  std::vector<cista::rtree<size_t>::rect> rand_rects_list;
  srand(static_cast<unsigned>(time(nullptr)));
